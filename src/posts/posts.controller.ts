...
export class PostsController {
  ...
  // ── Public read — no throttle needed (GET, read-only) ────────
  ...
  // ── Like — 3 per minute per IP (prevent like bombing) ────────
  ...
  async ogProxy(@Param('slug') slug: string, @Res() res: Response) {
    ...
    try {
      ...
      // fall through — render site defaults
    }
...
<body>
  ...
  // ── Admin endpoints (JWT protected — throttle skipped) ───────
